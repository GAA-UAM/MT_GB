...
class BaseMTGB(BaseGradientBoosting):
    ...
    def _init_state(self):
        ...
        self.estimators_ = np.empty(
            (self.n_estimators, self.T, self._loss.K), dtype=object)
    ...
    def _fit_stage(self,
        ...
        for k in range(loss.K):
            ...
            if i >= self.n_common_estimators:
                ...
                # We use the 0-th estimator for all tasks
                self.estimators_[i, 0, k] = tree
    ...
    def _raw_predict_init(self, X):
        ...
        if self.n_common_estimators > 0:
            # We use the 0-th index (common estim) for the validation
            X_data = self.estimators_[0, 0, 0]._validate_X_predict(
                X_data, check_input=True)
        else:
            for r_label in unique:
                idx_r = (t == r_label)
                X_data[idx_r] = self.estimators_[0, self.tasks_dic[r_label],
                                                 0]._validate_X_predict(X_data[idx_r], check_input=True)
        if self.init_ == 'zero':
            raw_predictions = np.zeros(shape=(X.shape[0], self._loss.K),
                                       dtype=np.float64)
        else:
    ...
    def _raw_predict(self, X):
        ...
        raw_predictions = self._raw_predict_init(
            X)  # init is common for all tasks
            ...
            if i >= self.n_common_estimators:
                for r_label in unique:
                    if r_label not in self.tasks_dic:
                        raise ValueError(
                            "The task {} was not present in the training set".format(r_label))
                    tree = self.estimators_[i, self.tasks_dic[r_label], 0]
                    idx_r = (t == r_label)
                    raw_predictions[idx_r] += (self.learning_rate *
                                               tree.predict(X[idx_r]))
            else:
                # The common estimator is in the 0-th index
                tree = self.estimators_[i, 0, 0]
